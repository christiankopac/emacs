...
;; Configure journal subdirectory
(with-eval-after-load 'denote
  (setq denote-journal-extras-directory 
        (expand-file-name "journal/daily/" denote-directory))
  
  ;; Use built-in journal commands
  (global-set-key (kbd "C-c j d") #'denote-journal-extras-new-or-existing-entry))

;; Capture-style journal log entry function
(defvar my/journal-capture-buffer "*Journal Log Entry*"
  "Buffer name for journal log capture.")

(defun my/journal-log-entry ()
  "Capture-style log entry for today's daily journal with Org timestamp."
  (interactive)
  (let* ((timestamp (format-time-string "[%Y-%m-%d %a %H:%M]"))  ; Org inactive timestamp
         (capture-buffer (get-buffer-create my/journal-capture-buffer)))
    ...
    (with-current-buffer capture-buffer
      ...
      (insert "\n\n")
      (insert "# Type your log entry above\n")
      (insert "# C-c C-c to save and close\n")
      (insert "# C-c C-k to cancel\n")
      (goto-char (point-min))
      (end-of-line)
      
      ;; Set up keybindings for this buffer
      (local-set-key (kbd "C-c C-c") 'my/journal-log-entry-finish)
      (local-set-key (kbd "C-c C-k") 'my/journal-log-entry-cancel))
    
    ;; Show capture buffer in a small window
    (delete-other-windows)
    (split-window-vertically -12)
    (other-window 1)
    (switch-to-buffer capture-buffer)
    (message "Type entry, then C-c C-c to save or C-c C-k to cancel")))

(defun my/journal-log-entry-finish ()
  "Save the journal log entry and close capture buffer."
  (interactive)
  (let* ((entry-text (with-current-buffer my/journal-capture-buffer
                       (goto-char (point-min))
                       (let ((end-pos (if (search-forward "\n\n#" nil t)
                                          (match-beginning 0)
                                        (point-max))))
                         (buffer-substring-no-properties (point-min) end-pos))))
         (today (format-time-string "%Y%m%dT000000"))
         (title (format-time-string "%A %e %B %Y"))
         (week-num (format-time-string "W%V"))
         (weekday (downcase (format-time-string "%A")))
         (month (downcase (format-time-string "%B")))
         (day (format-time-string "%-d"))
         (filename (format "%s--%s-%s-%s-%s__daily_journal.org" today week-num weekday month day))
         (journal-dir (if (boundp 'denote-journal-directory)
                          denote-journal-directory
                        (expand-file-name "journal/daily" "~/Sync/org")))
         (journal-file (expand-file-name filename journal-dir)))
    ...
    (with-current-buffer (find-file-noselect journal-file)
          ...
          (progn
            ...
            (let ((last-entry-pos (point)))
              ...
              (if (looking-at "^\\*\\*\\* ")
                ...
                ;; No existing entries
                (newline))))
        ;; Log section doesn't exist, create it
        (if (search-forward "* Daily Notes" nil t)
            (progn
              (end-of-line)
              (newline 2)
              (insert "** Log\n\n"))
      ...
      ;; Insert the entry
      (insert (string-trim entry-text))
      (unless (bolp) (newline))
      (save-buffer))
    
    ;; Close capture buffer and restore window
    (kill-buffer my/journal-capture-buffer)
    (delete-window)
    (message "Journal entry saved!")))

(defun my/journal-log-entry-cancel ()
  "Cancel journal log entry without saving."
  (interactive)
  (kill-buffer my/journal-capture-buffer)
  (delete-window)
